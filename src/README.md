...
在 IP 规则（如 `telegramip`、`googleip`、`privateip` 等）中均启用了 `,no-resolve` 标记。
...
* 优先通过 `geolocation-cn` 和 `cnip` 规则将绝大多数国内已知网站、APP 及大陆 IP 剥离，确保**国内流量 100% 直连**，不占用代理带宽。
* 尾部采用 `MATCH,🐠漏网之鱼` 进行防御性兜底，用户可在客户端中手动将此分组切换为直连或代理，灵活控制兜底策略。

### 4. 规则集直连拉取，避免代理环路

所有 `rule-providers` 和 `proxy-providers` 均配置 `proxy: DIRECT`，规则文件和订阅的下载请求绕过代理直连：

```yaml
rule-providers:
  cn:
    type: http
    proxy: DIRECT        # 直连拉取，不占用代理带宽
    url: https://testingcf.jsdelivr.net/gh/MetaCubeX/...
```

既避免了"需要代理才能下载规则，但规则没下载就无法代理"的死循环，也节省了代理节点的流量消耗。
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>YouTube、Steam、AI 等高频服务已前置分流，开箱即用</td>
        </tr>
        <tr>
