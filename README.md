...
> [!TIP]  
> ### 🎯 配置文件特点
> * **[📄 配置文件地址](/src/example.yaml)**
>
> * **[📄 分流规则详解](/src/README.md)**
> #### 1. 杜绝 DNS 泄露
>
> 国内域名使用国内DNS（阿里云、腾讯云）通过直连解析，境外域名使用公共 DNS 通过代理解析，二者严格隔离。FakeIP 模式进一步将真实 IP 隐藏在内核层，避免 DNS 查询绕过代理泄露用户位置信息。参考多份配置文件，经实际验证，无DNS泄露风险。
>
> #### 2. 国家分组
>
> 节点自动按国家归类——**香港、日本、韩国、美国、新加坡、台湾省**，剩余节点归入「其他节点」。
>
> #### 3. 国内直连 + 国外代理，自动分流
>
